...
/* ------------------------------------------------------------------ */
/* Preview search — Apple's public iTunes Search API.                 */
/* This is the one legitimate way to pull in real, recognizable songs */
/* here: 30-second preview clips, not full tracks, no key needed.     */
/* Apple's own CORS support is inconsistent from a browser, so this   */
/* tries a direct call first and falls back to a public CORS proxy.   */
/* ------------------------------------------------------------------ */
function normalizeItunesResults(data) {
  return (data.results || [])
    ...
    .map((r) => ({
      ...
      cover: (r.artworkUrl100 || "").replace("100x100", "400x400"),
      previewUrl: r.previewUrl,
      storeUrl: r.trackViewUrl || "",
    }));
}

async function searchSongPreviews(term) {
  const target = `https://itunes.apple.com/search?media=music&entity=song&limit=12&term=${encodeURIComponent(term)}`;
  try {
    const res = await fetch(target);
    if (!res.ok) throw new Error("direct fetch failed");
    return normalizeItunesResults(await res.json());
  } catch {
    try {
      ...
      return null; // both attempts failed — let the caller show a message
    }
  }
}
        ...
        <div key={s.id} className={`flex items-center gap-3 px-3 py-2.5 ${i !== 0 ? "border-t border-slate-200" : ""} hover:bg-violet-50 group`}>
          ...
          <div className="min-w-0 flex-1">
            <div className="text-sm truncate flex items-center gap-1.5">
              {s.title}
              {s.isPreviewClip && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-violet-100 border border-slate-300 text-slate-500 shrink-0">0:30 preview</span>}
              {s.hasUploadedAudio && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-violet-100 border border-slate-300 text-slate-500 shrink-0">uploaded</span>}
            </div>
            <div className="text-xs text-slate-500 truncate">
              {s.artist}{s.album ? ` · ${s.album}` : ""}
              {s.storeUrl && <a href={s.storeUrl} target="_blank" rel="noreferrer" className="text-amber-600 hover:text-amber-700 ml-1.5" onClick={(e) => e.stopPropagation()}>Full song ↗</a>}
            </div>
          </div>
...
function RightsNotice() {
  return (
    <div className="flex gap-2 text-xs text-amber-900/90 bg-amber-50 border border-amber-300 rounded-lg p-2.5">
      <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
      <span>Only add music you actually have the rights to use — your own recordings, royalty-free tracks, or anything Creative Commons licensed. Don't link copyrighted songs you don't have permission for.</span>
    </div>
  );
}
...
function AddSongModal({ user, onClose, onSaved, flash }) {
  const [source, setSource] = useState("search"); // search | upload | manual
  const [title, setTitle] = useState("");
  const [artist, setArtist] = useState("");
  ...
  function resetFields() {
    ...
    setCoverPreviewSrc(""); setCoverBlob(null); setStoreUrl(""); setFromPreview(false);
  ...
  async function runSearch(e) {
    ...
    const results = await searchSongPreviews(searchTerm.trim());
    setSearching(false);
    if (results === null) { setSearchFailed(true); setSearchResults([]); }
    else setSearchResults(results);
  }

  function pickPreview(track) {
    setTitle(track.title);
    setArtist(track.artist);
    setAlbum(track.album);
    setAudioUrl(track.previewUrl);
    setCoverPreviewSrc(track.cover);
    setCoverBlob(null); // this cover is already a permanent URL — nothing to upload
    setStoreUrl(track.storeUrl);
    setFromPreview(true);
    setFromUpload(false);
    setConfirmed(true); // Apple's own preview clip — the rights question doesn't apply the way it does to a link someone found elsewhere
    setSource("manual"); // hop to the review screen so they can see + tweak what was picked
  }
  ...
  async function handleSave(e) {
    ...
    if (!fromUpload && !audioUrl.trim()) { setError("Add a direct audio link, pick a track from Search, or upload a file."); return; }
    if (fromUpload && !audioFile) { setError("Upload an audio file first."); return; }
    if (!confirmed) { setError("Please confirm you have the rights to use this audio."); return; }

    setBusy(true);
    try {
      // A cover pulled straight from Apple's catalog doesn't need the vision
      // check re-run — only check images the user actually uploaded.
      const coverForCheck = !fromPreview && coverBlob ? await blobToDataUrl(coverBlob) : "";
      ...
      const song = {
        ...
        storeUrl: storeUrl || "",
        isPreviewClip: fromPreview,
        hasUploadedAudio: fromUpload,
        vibes,
        owner: user.username,
        status: user.isAdmin ? "approved" : "pending",
        createdAt: Date.now(),
        rightsConfirmedBy: user.username,
    ...
    <ModalShell title="Add a song" onClose={onClose} wide>
      <div className="flex gap-1 mb-4 bg-violet-100 rounded-lg p-1">
        <button
          ...
          className={`flex-1 py-1.5 rounded-md text-xs sm:text-sm font-medium transition ${source === "search" ? "bg-amber-400 text-slate-950" : "text-slate-500"}`}
        >
          Search previews
        </button>
        <button
      ...
      {source === "search" ? (
        <div className="space-y-3">
          <div className="text-xs text-slate-500">
            Searches Apple's public song catalog and pulls a real, official 30-second preview clip for
            each result — that's the piece of a commercial song this project is actually allowed to stream
            without a label license. Picking one lets you review and save it on the next screen.
          </div>
          <form onSubmit={runSearch} className="flex gap-2">
            <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={inputCls} placeholder="Search a song or artist..." />
            <button disabled={searching} className="shrink-0 bg-amber-400 hover:bg-amber-300 text-slate-950 rounded-lg px-3 disabled:opacity-60">
              {searching ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
            </button>
          </form>

          {searchFailed && (
            <div className="text-xs text-amber-900/90 bg-amber-50 border border-amber-300 rounded-lg p-2.5 flex gap-2">
              <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
              <span>The preview search didn't come back. Try again, or use "Upload a file" or "Paste a link" instead.</span>
            </div>
          )}

          {searchResults && searchResults.length > 0 && (
            <div className="max-h-72 overflow-y-auto border border-slate-300 rounded-lg divide-y divide-slate-200">
              {searchResults.map((r) => (
                <button
                  type="button"
                  key={r.id}
                  onClick={() => pickPreview(r)}
                  className="w-full flex items-center gap-3 p-2.5 hover:bg-violet-50 text-left"
                >
                  <img src={r.cover || FALLBACK_COVER} className="w-9 h-9 rounded object-cover shrink-0" alt="" />
                  <div className="min-w-0 flex-1">
                    <div className="text-sm truncate">{r.title}</div>
                    <div className="text-xs text-slate-500 truncate">{r.artist} · {r.album}</div>
                  </div>
                  <span className="text-[11px] text-amber-600 font-semibold shrink-0">Use this</span>
                </button>
              ))}
            </div>
          )}
          {searchResults && searchResults.length === 0 && !searchFailed && (
            <div className="text-xs text-slate-500">No preview-able results for that search — try a different spelling or artist.</div>
          )}
        </div>
      ) : source === "upload" ? (
        <div className="space-y-3">
          <RightsNotice />
          <div className="text-xs text-slate-500">
            Upload any audio or video file from your own computer — mp3, wav, m4a, mp4, anything with a
            playable audio track — as long as you have the rights to use it (your own recording is the
            easy case). Video files will play if the browser can decode their audio track, but audio-only
            formats like mp3 are the most reliable.
          </div>
        ...
        <form onSubmit={handleSave} className="space-y-3">
          {fromPreview ? (
            <div className="flex gap-2 text-xs text-amber-900/90 bg-amber-50 border border-amber-300 rounded-lg p-2.5">
              ...
              <span>
                This is a 30-second official preview clip from Apple's catalog, not the full song.
                {storeUrl && <> A link to the full track on the iTunes Store will be shown with it.</>}
              </span>
            </div>
          ) : fromUpload ? (
            <div className="flex gap-2 text-xs text-slate-500 bg-violet-100 border border-slate-300 rounded-lg p-2.5">
              <Music2 className="w-4 h-4 shrink-0 mt-0.5 text-amber-500" />
              <span>Uploaded file: {uploadedFileName} (uploads when you save)</span>
            </div>
          ) : (
            <RightsNotice />
          )}
          ...
          {!fromPreview && !fromUpload && (
            <Field label="Audio link" hint="A direct URL to hosted audio you have the rights to use (e.g. a royalty-free site or your own hosting). Not a YouTube/Spotify/Apple Music page link.">
              <input value={audioUrl} onChange={(e) => setAudioUrl(e.target.value)} className={inputCls} placeholder="https://.../track.mp3" />
            </Field>
          )}
          {fromUpload && (
            <button type="button" onClick={() => setSource("upload")} className="text-xs text-amber-600 hover:underline">Replace the uploaded file</button>
          )}
          <Field label="Cover art (optional)">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm text-slate-500">{fromPreview ? "Replace image" : "Upload image"}</span>
              <input type="file" accept="image/*" onChange={handleCover} className="hidden" />
            </label>
            {!fromPreview && <div className="text-[11px] text-slate-500 mt-1">Checked automatically for school-appropriateness, and uploaded when you save.</div>}
          </Field>
          ...
          {!fromPreview && (
            <label className="flex items-start gap-2 text-xs text-slate-500 pt-1">
              ...
              I confirm I have the rights or permission to use this audio, and that the info above is accurate.
            </label>
          )}
          ...
          <div className="flex gap-2">
            {(fromPreview || fromUpload) && (
              <button type="button" onClick={() => { resetFields(); setSource(fromPreview ? "search" : "upload"); }} className="text-sm text-slate-500 hover:text-slate-700 px-3">
                Start over
              </button>
            )}
