...
add songs you actually have the rights to (upload your own audio file, or
paste a direct link), build playlists, search the whole class's library,
and stream through a real player with a queue. An admin account can
moderate or remove anything.
...
Two ways, both for music you have the rights to use: upload an audio (or
video) file straight from your computer, or paste a direct link to
already-hosted audio. Uploaded files are stored in Vercel Blob and streamed
back through an `<audio>` element, so even a video file (mp4) plays its
audio track only — no video is ever shown.
